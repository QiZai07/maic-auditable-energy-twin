...
The public artifact is rebuilt from deterministic synthetic hourly profiles generated by `scripts/build_public_demo_data.py`. The candidate gradient-boosting regressor remains rejected by the documented performance gate; the transparent calendar prior is selected.
